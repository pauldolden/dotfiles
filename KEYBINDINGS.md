...
- `g + c` - Go to ~/.config
- `g + d` - Go to ~/dev
- `g + v` - Go to the notes root (`yazi/keymap.toml`, edit for your own)
- `g + D` - Go to Downloads
...
### Notes
Machine-local — defined in `zsh/aliases.local.zsh`, not the shared set.
- `notes` / `vault` - Open the notes root in nvim
