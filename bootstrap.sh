...
step "Seeding machine-local files"
# Derived from the .example files that are tracked, not enumerated here, so a
# new machine-local file needs no edit to this script. Never overwrites: an
# existing target is this machine's own state, not a stale copy of the example.
while IFS= read -r ex; do
	...
	if [ -f "$CONFIG/$target" ]; then
		...
		cp "$CONFIG/$ex" "$CONFIG/$target"
		warn "$target created from its .example — review it"
	fi
done <<EOF
$(git -C "$CONFIG" ls-files '*.example')
EOF
...
if cat "$CONFIG/Brewfile" "$CONFIG/Brewfile.local" 2>/dev/null | brew bundle --file=-; then
...
if ! command -v brew >/dev/null; then
	...
	stale=$(cat "$CONFIG/Brewfile" "$CONFIG/Brewfile.local" 2>/dev/null |
		brew bundle cleanup --file=- 2>/dev/null |
		awk '/^Would uninstall (formulae|casks):/{f=1;next} /^Would |^Run /{f=0} f&&NF{c++} END{print c+0}' || true)
	if [ "$stale" -eq 0 ]; then
		skip "brew matches the Brewfile"
	else
		warn "$stale brew packages are in neither Brewfile — review 'cat $CONFIG/Brewfile $CONFIG/Brewfile.local | brew bundle cleanup --file=-', then re-run it with --force"
	fi
fi
...
cat <<'EOF'
    Remaining manual steps:
      ...
      gcloud init          your Google Cloud account, if you need one
