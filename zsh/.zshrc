# Ghostty ships its own terminfo. Downgrade to xterm-256color ONLY when that
# terminfo is absent — over SSH to a host that has never seen ghostty, or in a
# container. Downgrading unconditionally costs the capabilities ghostty's entry
# describes (styled underlines, extended keys) for no benefit locally.
...
# Machine-local overlay (gitignored, optional). Sourced after the shared set so
# a machine can override rather than collide — and so nothing naming one
# machine's projects, employer or apps has to live in the tracked files.
[[ -f ~/.config/zsh/aliases.local.zsh ]]   && source ~/.config/zsh/aliases.local.zsh
[[ -f ~/.config/zsh/functions.local.zsh ]] && source ~/.config/zsh/functions.local.zsh
...
path+=(
  ...
  $HOME/Library/pnpm
  $HOME/go/bin
  $ZSH_LOCAL_PATH        # set by zsh/aliases.local.zsh, empty if unset
)
...
# --------------------------------------------------
# Tmux — keep the window name tracking the working directory
# --------------------------------------------------
